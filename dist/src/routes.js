...
function createFallbackStorage() {
    ...
    return {
        ...
        updateAgentMetrics: async (id, metrics) => {
            console.log(`Fallback: Updating agent ${id} metrics`, metrics);
            return { id, metrics };
        },
        getDigitalTwins: async () => [],
        createDigitalTwin: async (data) => data,
        getConversations: async () => [],
        getConversation: async (id) => null,
        createConversation: async (data) => data,
        getConversationsByParticipant: async (name) => [],
        cleanupDuplicateAgents: async () => 0,
        createAgent: async (data) => data
    };
}
import { insertAgentSchema, insertDigitalTwinSchema, insertConversationSchema } from "./schema.js";
...
function isFallbackResponse(result) {
